...
LEVEL_COLS = [('PDH','prev_day_high'),('AH','asian_high'),('WH','weekly_high'),('VAH','vp_vah')]

def active_level(row):
    high=row['high']; levels=[]; best_px=None; best_dist=99
    for k,col in LEVEL_COLS:
        v=row.get(col)
        if not v or v<=0: continue
        if abs(high-v)/v > LEVEL_TOL: continue
        levels.append(k)
        dist=abs(row['close']-v)/v
        if dist < best_dist: best_dist=dist; best_px=float(v)
    if not levels: return False,'',None
    lbl='+'.join(levels)
    if 'VAH' not in lbl: return False,'',None
    parts=lbl.split('+')
    if len(parts)>=3 and 'PDH' in parts and 'AH' in parts: return False,'',None
    return True, lbl, best_px
...
def simulate(df, limit_offset=None, max_wait=0):
    """
    limit_offset=None  -> entrada al close (mercado, comportamiento actual)
    limit_offset=0.0   -> limite en el nivel exacto
    limit_offset=0.001 -> limite 0.1% por debajo del nivel
    max_wait           -> barras maximas esperando el fill del limite
    """
    ...
    trades=[]; in_t=False; pending=False
    ep=sl=tp=dist=limit_px=0.0; t_start=t_entry=wait_start=0
    mfe=mae=cvd_streak=0; cap=CAPITAL
    ...
    for i,row in enumerate(rows):
        ...
        if in_t:
            ...
            if reason:
                ...
                trades.append({'ts_ms':t_entry,'result_r':round(pnl_r,3),'oos':ts>=OOS_MS,'entry_px':round(ep,2),'level_px':round(limit_px,2)})
                cap+=pnl_usd; in_t=False; cvd_streak=0
            continue

        # gestionar orden limite pendiente
        if pending:
            if row['high'] >= limit_px:  # precio subio de nuevo al nivel — fill
                ep=limit_px
                h1d=h1_ctx.get((ts//H1_MS)*H1_MS)
                if h1d:
                    h1h,h1a=h1d; sl_=h1h+0.30*h1a; d=sl_-ep
                    if d>0:
                        sp=d/ep
                        if MIN_STOP<=sp<=MAX_STOP:
                            in_t=True; sl=sl_; dist=d; tp=ep-TARGET_R*dist
                            t_start=i; t_entry=ts; mfe=mae=cvd_streak=0
                            pending=False; continue
                pending=False  # stop invalido o fuera de rango
            elif i-wait_start >= max_wait:
                pending=False  # expiro la espera
            continue

        # condiciones de entrada
        sess=session(ts)
        if not sess: continue
        ok,lbl,level_px=active_level(row)
        if not ok: continue
        ...
        if limit_offset is None:
            # entrada al mercado (close)
            ep=row['close']; sl_=h1h+0.30*h1a; d=sl_-ep
            if d<=0: continue
            sp=d/ep
            if not (MIN_STOP<=sp<=MAX_STOP): continue
            in_t=True; sl=sl_; dist=d; tp=ep-TARGET_R*dist
            t_start=i; t_entry=ts; limit_px=level_px; mfe=mae=cvd_streak=0
        else:
            # orden limite: esperar retesteo del nivel
            limit_px=level_px * (1 - limit_offset)
            if row['high'] >= limit_px:  # fill inmediato si el high ya toco el nivel
                ep=limit_px; sl_=h1h+0.30*h1a; d=sl_-ep
                if d<=0: continue
                sp=d/ep
                if not (MIN_STOP<=sp<=MAX_STOP): continue
                in_t=True; sl=sl_; dist=d; tp=ep-TARGET_R*dist
...
print(f"{'Entrada':<26}  {'n_IS':>5}  {'WR_IS':>6}  {'AvgR_IS':>8}  {'n_OOS':>5}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*90)

configs = [
    (None,  0,  'Mercado (close)'),
    (0.000, 5,  'Limite en nivel   5b'),
    (0.000, 10, 'Limite en nivel  10b'),
    (0.001, 5,  'Limite -0.1%      5b'),
    (0.001, 10, 'Limite -0.1%     10b'),
    (0.002, 5,  'Limite -0.2%      5b'),
    (0.002, 10, 'Limite -0.2%     10b'),
]

for loff, mwait, label in configs:
    trades, cap = simulate(df, limit_offset=loff, max_wait=mwait)
    ...
    print(f"{label:<26}  {ni:>5}  {wi:>5.1f}%  {ai:>+.3f}     {no:>5}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}")
