...
def simulate(df, wh_block_15h=False, wh_require_sweep_or_abs=False, wh_block_thin_cvd=False):
    ...
    for i,row in enumerate(rows):
        ...
        # filtros especificos WH
        if has_wh:
            hm = (ts//60_000)%1440
            if wh_block_15h and 15*60<=hm<16*60: continue
            if wh_require_sweep_or_abs:
                sweep = bool(row.get('sweep_confirmed'))
                abs_a = bool(row.get('abs_ask'))
                if not sweep and not abs_a: continue
            if wh_block_thin_cvd:
                if bool(row.get('thin_below')) or bool(row.get('cvd_div')): continue
...
print(f"{'Config':<36}  {'n_tot':>6}  {'n_IS':>5}  {'WR_IS':>6}  {'n_OOS':>5}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*102)

configs = [
    (False, False, False, 'BASE v2'),
    (True,  False, False, '+ bloquear 15h en WH'),
    (False, True,  False, '+ sweep/abs en WH'),
    (False, False, True,  '+ bloquear thin/cvd en WH'),
    (True,  True,  False, '+ 15h + sweep/abs'),
    (True,  False, True,  '+ 15h + thin/cvd'),
    (False, True,  True,  '+ sweep/abs + thin/cvd'),
    (True,  True,  True,  'TODOS filtros WH'),
]

for b15, bsweep, bthin, label in configs:
    trades, cap = simulate(df, wh_block_15h=b15, wh_require_sweep_or_abs=bsweep, wh_block_thin_cvd=bthin)
    ...
    ntot=ni+no
    print(f"{label:<36}  {ntot:>6}  {ni:>5}  {wi:>5.1f}%  {no:>5}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}")
