"""
_calibrate_longs7.py — Ronda 6
WL+VAL OOS: WR 84.6%, AvgR +1.001 con n=13. Es el setup premium.
Objetivo: conseguir MAS trades WL+VAL sin degradar el sistema.
Opciones:
  1. Ampliar LEVEL_TOL solo para WL (1.0%, 1.2%, 1.4%)
  2. Aceptar WL sin VAL si hay confluencia con AL o PDL
  3. Explorar si VAL solo es recuperable con bloqueos adicionales
"""
...
def simulate(df, wl_tol=BASE_TOL, wl_req_val=True,
    ...
    for i,row in enumerate(rows):
        ...
        # Requiere VAL siempre, excepto WL con otra confluencia
        if wl_req_val:
            if 'VAL' not in lbl: continue
        else:
            # WL sin VAL permitido si hay AL o PDL
            if 'VAL' not in lbl and 'WL' not in lbl: continue
            if 'VAL' not in lbl and 'WL' in lbl:
                if not ('AL' in lbl or 'PDL' in lbl): continue
...
configs = [
    (0.007, True,  'BEST v5+PDL08 (referencia WL_TOL=0.70%)'),
    # Ampliar tolerancia WL
    (0.010, True,  'WL_TOL=1.00% (mas WL+VAL)'),
    (0.012, True,  'WL_TOL=1.20%'),
    (0.014, True,  'WL_TOL=1.40%'),
    (0.015, True,  'WL_TOL=1.50%'),
    (0.020, True,  'WL_TOL=2.00%'),
    # WL sin VAL requerido (con AL o PDL)
    (0.007, False, 'WL sin VAL (WL+AL o WL+PDL permitido)'),
    (0.010, False, 'WL_TOL=1.00% + WL sin VAL'),
]

for tol, req_val, label in configs:
    t,cap=simulate(df, wl_tol=tol, wl_req_val=req_val)
    ist=[x for x in t if not x['oos']]; oot=[x for x in t if x['oos']]
    ni,wi,_=s(ist); no,wo,ao=s(oot)
    flag = ' ***' if cap > 27000 else (' **' if cap > 25745 else '')
    print(f'{label:<58}  {ni:>4}  {wi:>5.1f}%  {no:>4}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}{flag}')

# Mostrar breakdown WL para tolerancias prometedoras
for tol, req_val, label in [(0.007,True,'v5+PDL08'), (0.010,True,'WL 1.0%'), (0.012,True,'WL 1.2%')]:
    t,_=simulate(df, wl_tol=tol, wl_req_val=req_val)
    wl_ts=[x for x in t if 'WL' in x['level']]
    oot_wl=[x for x in wl_ts if x['oos']]
    n=len(oot_wl); w=sum(1 for x in oot_wl if x['result_r']>0)
    avg=sum(x['result_r'] for x in oot_wl)/n if n else 0
    by_lbl=defaultdict(list)
    for x in oot_wl: by_lbl[x['level']].append(x['result_r'])
    print(f'\n  WL trades OOS [{label}] n={n}  WR={w/n*100:.1f}%  AvgR={avg:+.3f}')
    for lbl in sorted(by_lbl, key=lambda x:-len(by_lbl[x])):
        rs=by_lbl[lbl]; n2=len(rs); w2=sum(1 for r in rs if r>0)
        print(f'    {lbl:<25}  n={n2:>3}  WR={w2/n2*100:>5.1f}%  AvgR={sum(rs)/n2:>+.3f}')
