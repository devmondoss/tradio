...
def simulate(df, ah_block_16h=False, ah_block_obi=False, ah_block_abs_ask=False):
    ...
    for i,row in enumerate(rows):
        ...
        is_ah_vah = lbl == 'AH+VAH'

        if is_ah_vah:
            hour = (ts//3_600_000)%24
            obi  = float(row.get('obi10_mean') or 0)
            if ah_block_16h and hour == 16: continue
            if ah_block_obi and obi < -0.15: continue
            if ah_block_abs_ask and bool(row.get('abs_ask')): continue
...
print(f"{'Config':<34}  {'n_tot':>6}  {'WR_IS':>6}  {'n_OOS':>5}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*96)

configs = [
    (False, False, False, 'BASE v3'),
    (True,  False, False, '+ AH block 16h'),
    (False, True,  False, '+ AH block OBI<-0.15'),
    (False, False, True,  '+ AH block abs_ask'),
    (True,  True,  False, '+ 16h + OBI'),
    (True,  False, True,  '+ 16h + abs_ask'),
    (False, True,  True,  '+ OBI + abs_ask'),
    (True,  True,  True,  'TODOS filtros AH'),
]

for b16, bobi, bask, label in configs:
    trades, cap = simulate(df, ah_block_16h=b16, ah_block_obi=bobi, ah_block_abs_ask=bask)
    ...
    print(f"{label:<34}  {ni+no:>6}  {wi:>5.1f}%  {no:>5}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}")
