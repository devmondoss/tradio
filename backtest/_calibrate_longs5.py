"""
_calibrate_longs5.py — Ronda 4 de calibracion
Conclusiones de _analyze_longs_quality:
  - WL+VAL: microestructura no discrimina wins de losses, el edge es el NIVEL puro
  - AL+VAL 12h UTC: WR 40% (toxico, n=40) — no estaba bloqueado!
  - AL+VAL 07h UTC: WR 44.7% (n=94) — potencial bloqueo
  - AL+VAL 08h UTC: WR 58.1% — buena hora
  - PDL+VAL por hora: 11/12/14h muy toxico (<40%), 15/17/18h buenos

Test: bloqueos adicionales de AL+VAL y PDL+VAL por hora
"""
...
configs = [
    (BASE_VAL, BASE_AL,           set(),             'BASE v2 (VAL 9-11 + AL 11/17/19)'),
    # AL+VAL: anadir bloqueos
    (BASE_VAL, BASE_AL|{12},      set(),             '+ AL+VAL block 12h (40% WR, n=40)'),
    (BASE_VAL, BASE_AL|{7},       set(),             '+ AL+VAL block 07h (44.7% WR, n=94)'),
    (BASE_VAL, BASE_AL|{12,7},    set(),             '+ AL+VAL block 07+12h'),
    (BASE_VAL, BASE_AL|{12,13},   set(),             '+ AL+VAL block 12+13h'),
    (BASE_VAL, BASE_AL|{12,7,13}, set(),             '+ AL+VAL block 07+12+13h'),
    # PDL+VAL: ya tiene 55% WR global — probar bloquear horas malas
    (BASE_VAL, BASE_AL,           {11,12,14},        '+ PDL+VAL block 11+12+14h (<38% WR)'),
    (BASE_VAL, BASE_AL|{12},      {11,12,14},        '+ AL 12h + PDL 11+12+14h'),
    (BASE_VAL, BASE_AL|{12,7},    {11,12,14},        '+ AL 07+12h + PDL 11+12+14h'),
    # VAL: probar mas bloqueos London
    (BASE_VAL|{12}, BASE_AL,      set(),             '+ VAL block 12h tambien'),
    (BASE_VAL|{7},  BASE_AL,      set(),             '+ VAL block 07h'),
]
