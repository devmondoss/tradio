...
def simulate(df, block_hrs=set()):
    ...
    for i,row in enumerate(rows):
        ...
        if 'WL' in parts and (ts//3_600_000)%24 in block_hrs: continue
...
print(f"{'Config':<35}  {'n_IS':>4}  {'WR_IS':>6}  {'n_OOS':>4}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*92)

configs=[
    (set(),      'BASE + block PDL+AL+VAL'),
    ({12},       '+ WL block 12h UTC'),
    ({16},       '+ WL block 16h UTC'),
    ({7},        '+ WL block 07h UTC'),
    ({12,16},    '+ WL block 12h+16h'),
    ({7,12,16},  '+ WL block 07h+12h+16h'),
]

for hrs, label in configs:
    t, cap = simulate(df, block_hrs=hrs)
    ist=[x for x in t if not x['oos']]; oot=[x for x in t if x['oos']]
    ni,wi,_=s(ist); no,wo,ao=s(oot)
    print(f"{label:<35}  {ni:>4}  {wi:>5.1f}%  {no:>4}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}")
