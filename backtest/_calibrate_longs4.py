...
def simulate(df, block_val_hrs=set(), block_al_hrs=set()):
...
print(f"{'Config':<44}  {'n_IS':>4}  {'WR_IS':>6}  {'n_OOS':>4}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*98)

configs=[
    (set(),          set(),        'BASE v1'),
    ({9,10,11},      set(),        'VAL block 09+10+11h (London mid)'),
    ({9,10,11,18},   set(),        'VAL block 09+10+11+18h'),
    (set(),          {11,17,19},   'AL+VAL block 11+17+19h'),
    ({9,10,11},      {11,17,19},   'VAL 09-11 + AL+VAL 11+17+19h'),
    ({9,10,11,18},   {11,17,19},   'VAL 09-11+18 + AL+VAL 11+17+19h'),
    ({9,10,11},      {11,17},      'VAL 09-11 + AL+VAL 11+17h'),
    ({7,9,10,11},    {11,17,19},   'VAL 07+09-11 + AL+VAL 11+17+19h'),
]

for bv, ba, label in configs:
    t,cap=simulate(df, block_val_hrs=bv, block_al_hrs=ba)
    ist=[x for x in t if not x['oos']]; oot=[x for x in t if x['oos']]
    ni,wi,_=s(ist); no,wo,ao=s(oot)
    print(f'{label:<44}  {ni:>4}  {wi:>5.1f}%  {no:>4}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}')
