    ...
    for i,row in enumerate(rows):
        ...
        if in_t:
            ...
            if reason:
                ...
                trades.append({'ts_ms':t_entry,'result_r':round(pnl_r,3),'pnl_usd':round(pnl_usd,2),'oos':ts>=OOS_MS})
...
print(f"{'Target':<8}  {'n_IS':>5}  {'WR_IS':>6}  {'AvgR_IS':>8}  {'n_OOS':>5}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*80)
for target in [1.5, 2.0, 2.5, 3.0]:
    trades, cap = simulate(df, target_r=target)
    ...
    print(f"{target:<8.1f}  {ni:>5}  {wi:>5.1f}%  {ai:>+.3f}     {no:>5}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}")
