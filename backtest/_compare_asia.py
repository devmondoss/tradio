"""
_compare_asia.py
Sesion Asia 00-07 UTC — ¿agrega valor al sistema?
"""
...
BASE   = ['london','overlap','ny']
ASIA   = ['asia']
ALL    = ['asia','london','overlap','ny']

configs = [
    (BASE,  'London+Overlap+NY (base)'),
    (ASIA,  'Asia solo'),
    (ALL,   'Asia+London+Overlap+NY'),
]

print(f"\n{'Config':<30}  {'n_tot':>6}  {'WR_IS':>6}  {'WR_OOS':>6}  {'n_OOS':>5}  {'AvgR_OOS':>9}  {'tpd_OOS':>8}  {'Capital':>10}")
print('-'*110)

for sess_list, label in configs:
    trades, cap = simulate(df, sess_list)
    ...
    print(f"{label:<30}  {ni+no:>6}  {wi:>5.1f}%  {wo:>5.1f}%  {no:>5}  {ao:>+.3f}      {no/90:>5.1f}    ${cap:>9,.0f}")

# Desglose Asia por nivel y hora
print('\n-- Asia OOS: desglose por nivel --------------------------------')
trades_asia, _ = simulate(df, ALL)
oot_asia = [t for t in trades_asia if t['oos'] and t['sess'] == 'asia']
print(f"  Total Asia OOS: {len(oot_asia)} trades")
by_lbl = defaultdict(list)
for t in oot_asia: by_lbl[t['level']].append(t['result_r'])
...
print('\n-- Asia OOS: desglose por hora UTC -----------------------------')
from collections import defaultdict
by_hr = defaultdict(list)
for t in oot_asia:
    hr = (t['ts_ms']//3_600_000) % 24
    by_hr[hr].append(t['result_r'])
for hr in sorted(by_hr):
    rs = by_hr[hr]; n = len(rs); w = sum(1 for r in rs if r > 0)
    flag = ' <<' if w/n < 0.40 else (' **' if w/n > 0.55 else '')
