...
def active_level(row):
    ...
    lbl='+'.join(levels)
    if 'VAH' not in lbl: return False,''
    parts=lbl.split('+')
    if len(parts)>=3 and 'PDH' in parts and 'AH' in parts: return False,''
    return True, lbl

def rejection(row, max_wick=0.85):
    c,o,h,l=float(row['close']),float(row['open']),float(row['high']),float(row['low'])
    rng=h-l
    if rng<=0: return False
    wick_pct=(h-max(c,o))/rng
    return 0.30 < wick_pct < max_wick and c<=o

def simulate(df, block_doji=False, block_obi_extreme=False, require_absorption=False):
    ...
    for i,row in enumerate(rows):
        ...
        ok,lbl=active_level(row)
        if not ok: continue
        if not rejection(row, max_wick=0.85 if block_doji else 1.01): continue

        obi   = float(row.get('obi10_mean') or 0)
        delta = float(row.get('delta') or 0)

        if obi >= -0.05 and delta >= 0: continue    # flujo bearish base (obi<-0.05 OR delta<0)
        if block_obi_extreme and obi < -0.30: continue
        if require_absorption and delta < 0: continue
...
print(f"{'Config':<32}  {'n_IS':>5}  {'WR_IS':>6}  {'AvgR_IS':>8}  {'n_OOS':>5}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*98)

configs = [
    (False, False, False, 'Base (×0.40 ATR)'),
    (True,  False, False, '+ bloquear doji wick>85%'),
    (False, True,  False, '+ bloquear OBI < -0.30'),
    (False, False, True,  '+ solo absorcion delta>=0'),
    (True,  True,  False, '+ doji + OBI extremo'),
    (True,  False, True,  '+ doji + absorcion'),
    (False, True,  True,  '+ OBI + absorcion'),
    (True,  True,  True,  'TODOS los filtros'),
]

for doji, obi_ext, absorb, label in configs:
    trades, cap = simulate(df, block_doji=doji, block_obi_extreme=obi_ext, require_absorption=absorb)
    ...
    print(f"{label:<32}  {ni:>5}  {wi:>5.1f}%  {ai:>+.3f}     {no:>5}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}")
