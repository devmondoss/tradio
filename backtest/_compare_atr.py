    ...
    for i,row in enumerate(rows):
        ...
        if in_t:
            ...
            if reason:
                ...
                trades.append({'ts_ms':t_entry,'result_r':round(pnl_r,3),'stop_pct':round(dist/ep*100,3),'oos':ts>=OOS_MS})
        ...
        h1h,h1a=h1d; sl_=h1h+atr_mult*h1a; d=sl_-row['close']
        if d<=0: continue
        sp=d/row['close']
...
print(f"{'ATR mult':<10}  {'n_IS':>5}  {'WR_IS':>6}  {'AvgR_IS':>8}  {'n_OOS':>5}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'stop_avg':>9}  {'Capital':>10}")
print('-'*96)

for mult in [0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]:
    trades, cap = simulate(df, atr_mult=mult)
    ...
    def s(ts):
        ...
        sp=sum(t['stop_pct'] for t in ts)/n
        return n, w/n*100, sum(t['result_r'] for t in ts)/n, sp
    ni,wi,ai,si=s(ist); no,wo,ao,so=s(oot)
    print(f"×{mult:<9.2f}  {ni:>5}  {wi:>5.1f}%  {ai:>+.3f}     {no:>5}  {wo:>5.1f}%  {ao:>+.3f}      {so:>6.3f}%   ${cap:>9,.0f}")
