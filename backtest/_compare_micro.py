"""
_compare_micro.py
Test de mejoras de microestructura — NO aplica a mtf_basics.py
"""
...
def simulate(df, block_thin_above=False, block_abs_bid=False, dynamic_target=False, block_near5_ask=False):
    h1 = resamp(df, H1_MS)
    ...
    for i, row in enumerate(rows):
        ...
        # ── Filtros microestructura de entrada ─────────────────────────────────
        if block_thin_above and bool(row.get('thin_above')): continue
        if block_abs_bid    and bool(row.get('abs_bid')):    continue
        if block_near5_ask  and float(row.get('near5_ask') or 0) > 0.8: continue
        ...
        # ── Target dinámico ────────────────────────────────────────────────────
        tgt = 3.0 if (dynamic_target and bool(row.get('thin_below'))) else TARGET_R
...
configs = [
    (False, False, False, False, 'BASE v5'),
    (True,  False, False, False, '+ block thin_above'),
    (False, True,  False, False, '+ block abs_bid'),
    (False, False, False, True,  '+ block near5_ask>0.8'),
    (False, False, True,  False, '+ target 3R si thin_below'),
    (True,  True,  False, False, '+ thin_above + abs_bid'),
    (True,  False, True,  False, '+ thin_above + target3R'),
    (True,  True,  True,  False, '+ thin_above + abs_bid + target3R'),
]

print(f"\n{'Config':<38}  {'n_tot':>6}  {'WR_IS':>6}  {'WR_OOS':>6}  {'n_OOS':>5}  {'AvgR_OOS':>9}  {'tpd':>5}  {'Capital':>10}")
print('-'*108)

for ta, ab, dt, na, label in configs:
    trades, cap = simulate(df, block_thin_above=ta, block_abs_bid=ab, dynamic_target=dt, block_near5_ask=na)
    ...
    print(f"{label:<38}  {ni+no:>6}  {wi:>5.1f}%  {wo:>5.1f}%  {no:>5}  {ao:>+.3f}      {no/90:>4.1f}  ${cap:>9,.0f}")
