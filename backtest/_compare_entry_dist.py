...
def active_level(row, entry_tol=None):
    high=row['high']; close=row['close']; levels=[]; best_px=None; best_dist=99
    for k,col in LEVEL_COLS:
        ...
        levels.append(k)
        dist=abs(close-v)/v
        if dist < best_dist: best_dist=dist; best_px=v
    ...
    if entry_tol is not None and best_dist > entry_tol: return False,'',None
...
def simulate(df, entry_tol=None):
    ...
    for i,row in enumerate(rows):
        ...
        ok,lbl,_=active_level(row, entry_tol=entry_tol)
        if not ok: continue
...
print(f"{'Entry tol':<12}  {'n_IS':>5}  {'WR_IS':>6}  {'AvgR_IS':>8}  {'n_OOS':>5}  {'WR_OOS':>6}  {'AvgR_OOS':>9}  {'Capital':>10}")
print('-'*82)
for tol, label in [(None,'sin filtro'), (0.0030,'0.30%'), (0.0020,'0.20%'), (0.0015,'0.15%'), (0.0010,'0.10%')]:
    trades, cap = simulate(df, entry_tol=tol)
    ...
    print(f"{label:<12}  {ni:>5}  {wi:>5.1f}%  {ai:>+.3f}     {no:>5}  {wo:>5.1f}%  {ao:>+.3f}      ${cap:>9,.0f}")
