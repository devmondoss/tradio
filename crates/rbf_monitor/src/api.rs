    ...
    match client.get(&url).send().await {
        Ok(r) if r.status().is_success() => {
            let raw: Vec<serde_json::Value> = match r.json().await {
                Ok(v) => v,
                Err(e) => { eprintln!("[api] parse error: {e}"); return vec![]; }
            };
            let bars: Vec<Bar> = raw.into_iter().filter_map(|entry| {
            ...
            eprintln!("[api] {} barras {} desde Binance", bars.len(), tf.interval());
            bars
        }
        Ok(r)  => { eprintln!("[api] binance status {}", r.status()); vec![] }
        Err(e) => { eprintln!("[api] binance error: {e}"); vec![] }
    }
}
...
pub async fn fetch_signals() -> Vec<RbfSignal> {
    ...
    match client.get(&url).headers(headers).send().await {
        ...
        Err(e) => { eprintln!("[api] signals: {e}"); vec![] }
    }
}
