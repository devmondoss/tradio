...
const AZERET_MONO_BYTES: &[u8] = include_bytes!("../../../assets/fonts/AzeretMono-Regular.ttf");
...
impl RbfMonitor {
    ...
    fn update(&mut self, msg: Message) -> Task<Message> {
        match msg {
            ...
            Message::TimeframeSelected(tf) => {
                ...
                self.timeframe = tf;
                self.bars.clear();
                return Task::future(async move { Message::BarsLoaded(api::fetch_bars(tf).await) });
            }
            Message::KlineEvent(ev) => match ev {
                KlineEvent::Connected => {
                    self.connected = true;
                }
                KlineEvent::Disconnected => {
    ...
    fn view(&self) -> Element<'_, Message> {
        let body = row![self.view_chart(), self.view_panels(),]
            .spacing(8)
            .height(Length::Fill);

        container(
            column![self.view_navbar(), body]
                .spacing(8)
                .height(Length::Fill),
        )
        .padding(10)
    ...
    fn view_navbar(&self) -> Element<'_, Message> {
        ...
        let status_str = if self.connected {
            "● live"
        } else {
            "● disconnected"
        };
    ...
    fn view_panels(&self) -> Element<'_, Message> {
        column![self.view_stats(), self.view_signals(), self.view_vetos(),]
            .spacing(8)
    ...
    fn view_signals(&self) -> Element<'_, Message> {
        let rows: Vec<Element<'_, Message>> = self
            ...
            .take(14)
            .map(|s| {
                let dir_color = if s.direction == "Long" {
                    T::UP
                } else {
                    T::DOWN
                };
    ...
    fn view_vetos(&self) -> Element<'_, Message> {
        ...
        let veto_rows: Vec<Element<'_, Message>> = top5
            .into_iter()
            .map(|(name, cnt)| {
                row![
                    text(name)
