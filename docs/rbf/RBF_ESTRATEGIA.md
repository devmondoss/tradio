...
> Fuente de verdad operativa: [RBF_REGLAS_ACTIVAS.md](RBF_REGLAS_ACTIVAS.md). Este documento explica la lógica y el contexto; si contradice la fuente operativa, corregir este documento.
...
|---------|---------------------|-------|
| BTCUSDT | **1** | Config calibrada app; evita moves ya maduros |
| ETHUSDT | **1** | Config calibrada app; London queda desactivado |
| BNBUSDT | **1** | Config calibrada app; evita selling consumido |
...
### 5e. OBI multi-depth y OBI intrabar

`obi_l10`, `obi_l20` y las muestras intrabar `obi_10s` se guardan para análisis, pero ya no suman puntos
como OBI alineado en el score live. El análisis live mostró que OBI en dirección del trade podía indicar
movimiento consumido. El flag activo es `obi_trap`: OBI contra el breakout, interpretado como participantes
atrapados.

Las muestras intrabar siguen siendo valiosas para responder si la presión fue sostenida o solo un spike al cierre,
pero su uso es shadow/data science hasta tener muestra suficiente.

**Score máximo actual: 6**. Flags activos: `stacked_imbalance`, `absorption`, `lvn_thin`, `vwap_bias`,
`oi_momentum`, `obi_trap`.


### 6. Cooldown

Una vez disparada una señal, el detector espera el `cooldown_bars` configurado en `[range_breakout]`.
Valor vigente: **60 barras M1**. El valor de 15 barras queda histórico para evitar overtrading M1.
...
El uso operativo actual no es "OBI alineado", sino `obi_trap`: participantes del lado equivocado atrapados por el breakout. `obi_l10`, `obi_l20` e `obi_10s` quedan como dataset para calibrar presión sostenida.
...
| `docs/rbf/RBF_REGLAS_ACTIVAS.md` | Fuente de verdad operativa: reglas vigentes, config, score, vetos |
| `docs/rbf/RBF_FEATURE_MATRIX.md` | Contrato de features para DS/DE |
