// =====================================================================
// micro_window.rs  —  Captura de micro-ventana por vela para DRR.
//
// Modelo: el buffer acumula TODA la vela en slots de 15s (20 slots para
// una vela de 5min). Al cerrar la vela (o al disparar un trigger) se
// seleccionan 5 slots segun el anclaje y se calculan los features.
//
// Wiring (marcado con TODO):
//   - Adapta `Trade` a tu struct real de trade del stream.
//   - Llama `on_trade` por cada trade; `on_candle_close` al cerrar la vela.
//   - Pasa atr / rango / zona desde tu `ctx` al construir el row.
//   - Inserta `MicroWindowRow` via tu supabase_writer.
// =====================================================================
...
pub struct MicroWindowConfig {
    ...
    pub candle_secs: u64,    // 300 para 5min
    pub capture_all_candles: bool, // true
}
...
impl MicroWindowConfig {
    fn window_ms(&self) -> u64 {
        self.bucket_count as u64 * self.bucket_secs * 1000
    }
    fn slot_ms(&self) -> u64 {
...
// ---------------------------------------------------------------------
// Trade de entrada (adapta a tu tipo real)
// ---------------------------------------------------------------------
pub struct Trade {
    pub ts_ms: i64,
    pub price: f64,
    pub size: f64,     // en unidades base o USD, se consistente
    pub is_buy: bool,  // true = agresor comprador (taker buy)
    pub is_big: bool,  // marcado por tu detector de big trades
    pub liq_usd: f64,  // liquidacion USD si este trade fue liquidacion, else 0
}
...
impl Slot {
    fn add(&mut self, t: &Trade) {
        ...
        if t.price > self.hi { self.hi = t.price; }
        if t.price < self.lo { self.lo = t.price; }
        if t.is_buy { self.buy_vol += t.size; } else { self.sell_vol += t.size; }
        self.trades += 1;
        if t.is_big { self.big_vol += t.size; }
        if t.size > self.max_trade { self.max_trade = t.size; }
        self.liq_usd += t.liq_usd;
    }
    fn vol(&self) -> f64 { self.buy_vol + self.sell_vol }
    fn delta(&self) -> f64 { self.buy_vol - self.sell_vol }
}

// ---------------------------------------------------------------------
// Buffer por simbolo/vela
// ---------------------------------------------------------------------
pub struct CandleMicroBuffer {
    cfg: MicroWindowConfig,
    candle_open_ms: i64,
    slots: Vec<Slot>,
    trigger_ms: Option<i64>, // si hubo trigger en esta vela
}
...
impl CandleMicroBuffer {
    ...
    /// Llamar cuando se detecta el trigger de la señal en esta vela.
    pub fn mark_trigger(&mut self, ts_ms: i64) {
        self.trigger_ms = Some(ts_ms);
    }

    /// Selecciona los indices de los 5 slots de la ventana segun anclaje.
    fn window_slot_indices(&self) -> (usize, usize) {
        ...
        match self.cfg.anchor {
            ...
            WindowAnchor::Trigger => {
                match self.trigger_ms {
                    Some(tms) => {
                        let off = (tms - self.candle_open_ms).max(0) as u64;
                        let ti = (off / self.cfg.slot_ms()) as usize;
                        let lead = n / 2; // 2 antes para n=5
                        let start = ti.saturating_sub(lead);
                        let end = (start + n).min(total);
                        let start = end.saturating_sub(n); // reajuste si pego al final
                        (start, end)
                    }
                    None => (total.saturating_sub(n), total), // fallback a cierre
                }
            }
        }
    }

    /// Construye el row al cerrar la vela.
    /// `ctx` aporta el contexto que el buffer no conoce (atr, rango, zona).
    pub fn build_row(&self, ctx: &MicroCtx, exchange: &str, symbol: &str) -> MicroWindowRow {
        let (s0, s1) = self.window_slot_indices();
        ...
        let aggressor_ratio = if win_vol_total > 0.0 { buy_vol / win_vol_total } else { 0.5 };
        ...
        let late_surge_ratio = if vol_mean > 0.0 { vols[n - 1] / vol_mean } else { 0.0 };
        ...
        let price_path_eff = if path_raw > 0.0 { price_net.abs() / path_raw } else { 0.0 };
        ...
        // --- DRR especifico (si hay rango) ---
        let (reclaimed, reclaim_bucket) = ctx.detect_reclaim(&win);
        let sweep_depth_atr = ctx.sweep_depth_atr(lo, hi);

        // --- JSONB con detalle por bucket ---
        let buckets_json = json!(win.iter().enumerate().map(|(i, s)| json!({
            "i": i,
            "buy_vol": s.buy_vol, "sell_vol": s.sell_vol, "delta": s.delta(),
            "vol": s.vol(), "trades": s.trades, "big_vol": s.big_vol,
            "max_trade": s.max_trade, "liq_usd": s.liq_usd,
        ...
        MicroWindowRow {
            ...
            candle_close_ms: self.candle_open_ms + (self.cfg.candle_secs * 1000) as i64,
            anchor: anchor_str(self.cfg.anchor).to_string(),
            window_start_ms,
            window_end_ms,
            bucket_count: self.cfg.bucket_count as i16,
            bucket_secs: self.cfg.bucket_secs as i16,
            in_drr_zone: ctx.in_drr_zone,
            range_location: ctx.range_location.clone(),
...
// ---------------------------------------------------------------------
// Contexto inyectado desde tu `ctx`
// ---------------------------------------------------------------------
pub struct MicroCtx {
    pub atr: f64,
...
impl MicroCtx {
    // Reclaim: el precio salio del rango y volvio dentro dentro de la ventana.
    fn detect_reclaim(&self, win: &[&Slot]) -> (Option<bool>, Option<i16>) {
        ...
        for (i, s) in win.iter().enumerate() {
            if s.lo < lo || s.hi > hi { went_out = true; }
    ...
    fn sweep_depth_atr(&self, win_lo: f64, win_hi: f64) -> Option<f64> {
        ...
        match self.range_location.as_deref() {
            Some("near_low") | Some("outside_low") => self.range_low.map(|rl| (rl - win_lo).max(0.0) / atr),
            Some("near_high") | Some("outside_high") => self.range_high.map(|rh| (win_hi - rh).max(0.0) / atr),
            _ => None,
        }
    }
}
...
// ---------------------------------------------------------------------
// Row de salida (para insertar en Supabase)
// ---------------------------------------------------------------------
...
pub struct MicroWindowRow {
    ...
    pub delta_b: [f64; 5], // mapear a delta_b0..b4 en el writer
    pub vol_b: [f64; 5],   // mapear a vol_b0..b4
    pub delta_slope: f64,
...
fn detect_flip(deltas: &[f64]) -> (bool, Option<i16>) {
    if deltas.is_empty() { return (false, None); }
...
fn classify_trajectory(vols: &[f64]) -> String {
    let n = vols.len();
    if n < 3 { return "flat".into(); }
    let peak = argmax(vols);
    let total: f64 = vols.iter().sum();
    let mean = total / n as f64;
    let spread = stdev(vols);
    if mean > 0.0 && spread / mean < 0.25 { return "flat".into(); }
    // u-shape: extremos altos, centro bajo
    let center = vols[n / 2];
    let ends = (vols[0] + vols[n - 1]) / 2.0;
    if center < ends * 0.6 { return "u_shape".into(); }
    match peak {
        0 | 1 => "front".into(),
...
fn pad5(v: &[f64]) -> [f64; 5] {
    let mut a = [0.0; 5];
    for (i, &x) in v.iter().take(5).enumerate() { a[i] = x; }
    a
}

fn anchor_str(a: WindowAnchor) -> &'static str {
    match a {
