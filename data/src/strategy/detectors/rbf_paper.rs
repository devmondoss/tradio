...
//!   - Sin time stop: la posición cierra por stop, target, trailing stop o fin de sesión.
...
impl RbfPaperTrader {
    ...
    /// `close` se conserva por compatibilidad con call sites; las salidas usan high/low.
    ...
    pub fn on_bar_close(
        ...
        match pos.direction {
            RbfDirection::Long => {
                if high > pos.best_extreme {
                    pos.best_extreme = high;
                }
                ...
                if fav_r >= trail_activate_r && !pos.trailing_active {
                    ...
                    if floor > pos.stop_price {
                        pos.stop_price = floor;
                    }
                    println!(
                        "[rbf_paper] trailing activado en {:.2}R (threshold={:.2}R)",
                        fav_r, trail_activate_r
                    );
                }
            ...
            RbfDirection::Short => {
                if low < pos.best_extreme {
                    pos.best_extreme = low;
                }
