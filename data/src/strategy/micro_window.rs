...
impl CandleMicroBuffer {
    ...
    pub fn reset(&mut self, candle_open_ms: i64) {
        ...
        for s in &mut self.slots { *s = Slot::default(); }
    }
    ...
    pub fn build_row(
        ...
        let monotonic_delta = deltas.iter().all(|d| *d >= 0.0)
        ...
        let buckets = json!(win.iter().enumerate().map(|(i, s)| json!({
            "i":         i,
...
impl MicroCtx {
    ...
    pub fn range_location_str(&self) -> Option<String> {
        self.range_loc.map(|loc| match loc {
    ...
    fn sweep_depth_atr(&self, win_lo: f64, win_hi: f64) -> Option<f64> {
        ...
        match self.range_loc {
            Some(RangeLocation::NearLow)    |
            Some(RangeLocation::OutsideLow)  =>
                self.range_low.map(|rl| (rl - win_lo).max(0.0) / atr),
            Some(RangeLocation::NearHigh)   |
            Some(RangeLocation::OutsideHigh) =>
