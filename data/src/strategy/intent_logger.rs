...
pub fn evaluate_lvn(ctx: &StrategyMarketContext, cfg: &StrategyConfig) -> Vec<NearMiss> {
    ...
    if ob.thin_zone_above {
        ...
        check(ob.thin_zone_above,                                                                  "thin_zone_above",       "no_thin_zone_above",    &mut met, &mut blocked);
        check(matches!(vw.price_vs_vwap, PriceRelation::Above | PriceRelation::At),               "vwap_above_or_at",      "below_vwap",            &mut met, &mut blocked);
        check(matches!(vp.value_location, ValueLocation::InValue | ValueLocation::AboveVah),       "value_location_ok",     "value_location_low",    &mut met, &mut blocked);
        check(flow.delta.unwrap_or(0.0) > 0.0,                                                    "delta_positive",        "delta_not_positive",    &mut met, &mut blocked);
        check(flow.cvd_slope.unwrap_or(0.0) > 0.0,                                                "cvd_slope_positive",    "cvd_slope_not_pos",     &mut met, &mut blocked);
        check(matches!(flow.stacked_imbalance, ImbalanceSide::Bullish | ImbalanceSide::None | ImbalanceSide::Unknown), "imbalance_ok", "imbalance_bearish", &mut met, &mut blocked);
        check(flow.taker_imbalance.unwrap_or(0.0).abs() < 0.90,                                   "taker_imbalance_ok",    "taker_extreme",         &mut met, &mut blocked);
        check(ob.spread_bps.unwrap_or(999.0) <= cfg.max_spread_bps,                               "spread_ok",             "spread_too_wide",       &mut met, &mut blocked);
        check(ob.microprice.map(|m| m >= px).unwrap_or(true),                                     "microprice_bullish",    "microprice_bearish",    &mut met, &mut blocked);
    ...
    if ob.thin_zone_below {
        ...
        check(ob.thin_zone_below,                                                                  "thin_zone_below",       "no_thin_zone_below",    &mut met, &mut blocked);
        check(matches!(vw.price_vs_vwap, PriceRelation::Below | PriceRelation::At),               "vwap_below_or_at",      "above_vwap",            &mut met, &mut blocked);
        check(matches!(vp.value_location, ValueLocation::InValue | ValueLocation::BelowVal),       "value_location_ok",     "value_location_high",   &mut met, &mut blocked);
        check(flow.delta.unwrap_or(0.0) < 0.0,                                                    "delta_negative",        "delta_not_negative",    &mut met, &mut blocked);
        check(flow.cvd_slope.unwrap_or(0.0) < 0.0,                                                "cvd_slope_negative",    "cvd_slope_not_neg",     &mut met, &mut blocked);
        check(matches!(flow.stacked_imbalance, ImbalanceSide::Bearish | ImbalanceSide::None | ImbalanceSide::Unknown), "imbalance_ok", "imbalance_bullish", &mut met, &mut blocked);
        check(flow.taker_imbalance.unwrap_or(0.0).abs() < 0.90,                                   "taker_imbalance_ok",    "taker_extreme",         &mut met, &mut blocked);
        check(ob.spread_bps.unwrap_or(999.0) <= cfg.max_spread_bps,                               "spread_ok",             "spread_too_wide",       &mut met, &mut blocked);
        check(ob.microprice.map(|m| m <= px).unwrap_or(true),                                     "microprice_bearish",    "microprice_bullish",    &mut met, &mut blocked);
...
pub fn evaluate_vwap(ctx: &StrategyMarketContext, cfg: &StrategyConfig) -> Vec<NearMiss> {
    ...
    let (long_anchor_ok, long_anchor_label) = if vw.price_vs_avwap_bos != PriceRelation::Unknown {
        (matches!(vw.price_vs_avwap_bos, PriceRelation::Above | PriceRelation::At), "anchor_avwap_bos")
    } else {
        (matches!(vw.price_vs_vwap, PriceRelation::Above | PriceRelation::At), "anchor_vwap_session")
    };

    let (short_anchor_ok, short_anchor_label) = if vw.price_vs_avwap_bos != PriceRelation::Unknown {
        (matches!(vw.price_vs_avwap_bos, PriceRelation::Below | PriceRelation::At), "anchor_avwap_bos")
    } else {
        (matches!(vw.price_vs_vwap, PriceRelation::Below | PriceRelation::At), "anchor_vwap_session")
    };
    ...
    let long_location = matches!(vp.value_location, ValueLocation::InValue | ValueLocation::BelowVal);
    ...
    if long_regime && (long_anchor_ok || long_location) {
        ...
        check(long_regime,                                                    "regime_bullish",        "regime_not_bullish",    &mut met, &mut blocked);
        check(long_anchor_ok,                                                 long_anchor_label,       "anchor_not_ok",         &mut met, &mut blocked);
        check(long_location,                                                  "pullback_into_value",   "not_in_value",          &mut met, &mut blocked);
        check(flow.cvd_slope.unwrap_or(0.0) >= 0.0,                          "cvd_slope_ok",          "cvd_slope_bearish",     &mut met, &mut blocked);
        check(flow.delta.unwrap_or(0.0) > 0.0,                               "delta_positive",        "delta_not_positive",    &mut met, &mut blocked);
        check(!flow.failed_acceptance,                                        "no_failed_acceptance",  "failed_acceptance",     &mut met, &mut blocked);
        check(ob.spread_bps.unwrap_or(999.0) <= cfg.max_spread_bps,          "spread_ok",             "spread_too_wide",       &mut met, &mut blocked);
        check(ob.microprice.map(|m| m >= px * 0.9998).unwrap_or(true),       "microprice_bullish",    "microprice_bearish",    &mut met, &mut blocked);
        ...
        results.push(NearMiss {
            ...
            atr: ctx.atr,
            met, blocked, score_pct,
            vah: Some(vah), val: Some(val), poc: vp.poc,
            delta: flow.delta, cvd_slope: flow.cvd_slope,
            vpin: flow.vpin, spread_bps: ob.spread_bps,
            failed_acceptance: flow.failed_acceptance,
        });
    ...
    let short_location = matches!(vp.value_location, ValueLocation::InValue | ValueLocation::AboveVah);
    ...
    if short_regime && (short_anchor_ok || short_location) {
        ...
        check(short_regime,                                                   "regime_bearish",        "regime_not_bearish",    &mut met, &mut blocked);
        check(short_anchor_ok,                                                short_anchor_label,      "anchor_not_ok",         &mut met, &mut blocked);
        check(short_location,                                                 "pullback_into_value",   "not_in_value",          &mut met, &mut blocked);
        check(flow.cvd_slope.unwrap_or(0.0) <= 0.0,                          "cvd_slope_ok",          "cvd_slope_bullish",     &mut met, &mut blocked);
        check(flow.delta.unwrap_or(0.0) < 0.0,                               "delta_negative",        "delta_not_negative",    &mut met, &mut blocked);
        check(!flow.failed_acceptance,                                        "no_failed_acceptance",  "failed_acceptance",     &mut met, &mut blocked);
        check(ob.spread_bps.unwrap_or(999.0) <= cfg.max_spread_bps,          "spread_ok",             "spread_too_wide",       &mut met, &mut blocked);
        check(ob.microprice.map(|m| m <= px * 1.0002).unwrap_or(true),       "microprice_bearish",    "microprice_bullish",    &mut met, &mut blocked);
