...
pub enum FootprintStudy {
    ...
    /// Volume-profile overlay (histogram + POC/VAH/VAL) over the visible candles.
    VolumeProfile,
    /// High/Low volume nodes (acceptance / rejection) markers.
    HvnLvn,
    /// Bearish/bullish delta divergence markers at price pivots.
    DeltaDivergence,
    /// Live orderflow confluence-detector marks (VP + wall + absorption).
    Confluence,
}
...
impl FootprintStudy {
    pub const ALL: [FootprintStudy; 6] = [
        ...
        FootprintStudy::HvnLvn,
        FootprintStudy::DeltaDivergence,
        FootprintStudy::Confluence,
    ];
}
...
impl std::fmt::Display for FootprintStudy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FootprintStudy::HvnLvn => write!(f, "HVN / LVN"),
            FootprintStudy::DeltaDivergence => write!(f, "Delta Divergence"),
            FootprintStudy::Confluence => write!(f, "Confluence"),
        }
    }
}
