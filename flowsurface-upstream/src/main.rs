...
struct Flowsurface {
    ...
    confluence: confluence::ConfluenceDetector,
}
...
impl Flowsurface {
    fn new() -> (Self, Task<Message>) {
        ...
        let mut state = Self {
            ...
            confluence: confluence::ConfluenceDetector::new(),
        };
            ...
            Message::MarketWsEvent(event) => {
                ...
                match event {
                    ...
                    exchange::Event::DepthReceived(stream, update_t, depth) => {
                        ...
                        self.confluence.on_depth(&stream, &depth);
                    ...
                    exchange::Event::TradesReceived(stream, update_t, buffer) => {
                        ...
                        if let Some(sig) = self.confluence.on_trades(&stream, &buffer) {
                            self.notifications.push(Toast::custom(
                                sig.title,
                                sig.body,
                                widget::toast::Status::Success,
                            ));
                        }
