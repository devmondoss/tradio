...
/// Aggregated volume profile over the visible candles (for the on-chart overlay).
struct VpOverlay {
    levels: Vec<(Price, f64)>,
    poc: Price,
    vah: Price,
    val: Price,
    max_vol: f64,
...
impl KlineChart {
    /// Sum the visible candles' footprints into a volume profile: POC + 70% value area.
    fn visible_vp(&self, earliest: u64, latest: u64) -> Option<VpOverlay> {
        let mut map: std::collections::HashMap<Price, f64> = std::collections::HashMap::new();
        let mut add = |trades: &KlineTrades| {
            for (price, g) in &trades.trades {
                *map.entry(*price).or_insert(0.0) += g.total_qty().to_f64();
            }
        };
        match &self.data_source {
            PlotData::TickBased(tick) => {
                let (e, l) = (earliest as usize, latest as usize);
                tick.datapoints
                    .iter()
                    .rev()
                    .enumerate()
                    .filter(|(i, _)| *i <= l && *i >= e)
                    .for_each(|(_, dp)| add(&dp.footprint));
            }
        ...
        let mut levels: Vec<(Price, f64)> = map.into_iter().collect();
        levels.sort_by(|a, b| a.0.cmp(&b.0));
        let total: f64 = levels.iter().map(|(_, v)| *v).sum();
        let max_vol = levels.iter().map(|(_, v)| *v).fold(0.0_f64, f64::max);
        let poc_idx = levels
            .iter()
            .enumerate()
        ...
        while acc < 0.70 * total && (lo > 0 || hi < levels.len() - 1) {
            let up = if hi + 1 < levels.len() { levels[hi + 1].1 } else { -1.0 };
            let dn = if lo > 0 { levels[lo - 1].1 } else { -1.0 };
            if up >= dn {
                hi += 1;
                acc += up.max(0.0);
            } else {
    ...
    /// Bearish/bullish delta divergences at price pivots over the visible range.
    /// Returns (x, price, is_bearish). Time-based charts only (chronological order).
    fn visible_divergences(
        &self,
        earliest: u64,
        latest: u64,
        ...
        let bars: Vec<(f32, Price, Price, f64)> = ts
            .datapoints
            .range(UnixMs::new(earliest)..=UnixMs::new(latest))
            .map(|(t, dp)| {
                (
                    interval_to_x(t.as_u64()),
                    dp.kline.high,
                    dp.kline.low,
                    dp.volume_delta().to_f64(),
                )
            })
            .collect();
        let n = bars.len();
        ...
        let mut out = Vec::new();
        let mut last_ph: Option<(f64, f64)> = None; // (pivot-high price, cum delta there)
        let mut last_pl: Option<(f64, f64)> = None; // (pivot-low price, cum delta there)
        for i in 1..n - 1 {
            let hi = bars[i].1.to_f64();
            let lo = bars[i].2.to_f64();
            if hi > bars[i - 1].1.to_f64() && hi >= bars[i + 1].1.to_f64() {
                if let Some((ph, pc)) = last_ph
                    && hi > ph
                    && cum[i] < pc
                {
                    out.push((bars[i].0, bars[i].1, true)); // higher high, weaker delta
                }
                last_ph = Some((hi, cum[i]));
            }
            if lo < bars[i - 1].2.to_f64() && lo <= bars[i + 1].2.to_f64() {
                if let Some((pl, pc)) = last_pl
                    && lo < pl
                    && cum[i] > pc
                {
                    out.push((bars[i].0, bars[i].2, false)); // lower low, weaker delta
                }
                last_pl = Some((lo, cum[i]));
            }
        }
        out
    }
...
/// Draw the volume-profile histogram on the right edge + POC/VAH/VAL lines.
fn draw_vp_overlay(
    ...
    // Volume-by-price histogram on the right edge (out of the way of centre clusters).
    if show_histogram {
        let right = vis_left + vis_width;
        let max_hist = vis_width * 0.12;
        for (price, vol) in &vp.levels {
            let w = ((vol / vp.max_vol) as f32) * max_hist;
            if w <= 0.0 {
                continue;
            }
            let y = price_to_y(*price);
            let in_va = *price >= vp.val && *price <= vp.vah;
            let alpha = if in_va { 0.55 } else { 0.22 };
            frame.fill_rectangle(
                Point::new(right - w, y - bar_h / 2.0),
                Size::new(w, bar_h),
    ...
    if show_hvnlvn {
    // HVN / LVN — local peaks (acceptance, S/R) and troughs (rejection, fast zones)
    let w = 2usize;
    let n = vp.levels.len();
    for i in 0..n {
        let (price, vol) = vp.levels[i];
        let lo = i.saturating_sub(w);
        let hi = (i + w).min(n - 1);
        if hi - lo < 2 * w {
            continue; // need full neighbourhood (skip edges)
        }
        let window = &vp.levels[lo..=hi];
        let y = price_to_y(price);
        if window.iter().all(|(_, v)| vol >= *v) && vol >= 0.5 * vp.max_vol {
            // HVN: solid tick at the left edge
            frame.fill_rectangle(
                Point::new(vis_left, y - bar_h / 2.0),
                Size::new(vis_width * 0.02, bar_h),
                palette.success.base.color.scale_alpha(0.85),
            );
        } else if window.iter().all(|(_, v)| vol <= *v) && vol <= 0.15 * vp.max_vol {
            // LVN: thin tick (price passes through fast)
            frame.fill_rectangle(
                Point::new(vis_left, y - 0.75),
                Size::new(vis_width * 0.02, 1.5),
                palette.danger.base.color.scale_alpha(0.7),
            );
        }
    }
    ...
    for (price, color, alpha) in [
        (vp.poc, palette.secondary.strong.color, 0.9_f32),
        (vp.vah, palette.secondary.base.color, 0.5),
        (vp.val, palette.secondary.base.color, 0.5),
    ] {
        let y = price_to_y(price);
        frame.fill_rectangle(
            Point::new(vis_left, y - 0.5),
            Size::new(vis_width, 1.0),
        ...
        let klines = chart.cache.main.draw(renderer, bounds_size, |frame| {
            ...
            match &self.kind {
                KlineChartKind::Footprint {
                    ...
                    let has = |want: FootprintStudy| studies.iter().any(|s| s.is_same_type(&want));
                    let show_vp = has(FootprintStudy::VolumeProfile);
                    let show_hvn = has(FootprintStudy::HvnLvn);
                    if (show_vp || show_hvn)
                        && let Some(vp) = self.visible_vp(earliest, latest)
                    {
                        draw_vp_overlay(
                            frame,
                            &price_to_y,
                            ...
                            region.width,
                            &vp,
                            chart.cell_height * 0.8,
                            palette,
                            show_vp, // histogram
                            show_vp, // POC/VAH/VAL lines
                            show_hvn,
                        );
                    }

                    // Delta divergence markers (study-gated)
                    if has(FootprintStudy::DeltaDivergence) {
                        for (x, price, bearish) in
                            self.visible_divergences(earliest, latest, &interval_to_x)
                        {
                            let y = price_to_y(price);
                            let (dy, color) = if bearish {
                                (-9.0, palette.danger.base.color)
                            } else {
                                (9.0, palette.success.base.color)
                            };
                            frame.fill_rectangle(
                                Point::new(x - 3.0, y + dy - 3.0),
                                Size::new(6.0, 6.0),
                                color.scale_alpha(0.9),
                            );
                        }
                    }

                    // Confluence-detector marks (study-gated)
                    if has(FootprintStudy::Confluence) {
                        let symbol = chart.ticker_info.ticker.to_string();
                        let since_ms = latest.saturating_sub(1_800_000);
                        for m in crate::confluence::recent_marks(&symbol, since_ms) {
                            let x = interval_to_x(m.at_ms);
                            let y = price_to_y(Price::from_f64(m.price));
                            let color = if m.bearish {
                            ...
                            frame.fill_rectangle(
                                ...
                                Size::new(8.0, 8.0),
                                color.scale_alpha(0.95),
                            );
                            let stalk_y = if m.bearish { y - 24.0 } else { y + 6.0 };
                            frame.fill_rectangle(
                                Point::new(x - 1.0, stalk_y),
                                Size::new(2.0, 18.0),
                ...
                KlineChartKind::Candles => {
                    ...
                    // Volume Profile on candles: here it adds the volume-by-price
                    // dimension the candles lack (not redundant like on footprint).
                    // Needs trade-level data; silently no-ops if the candle chart
                    // has no footprint accumulated.
                    if let Some(vp) = self.visible_vp(earliest, latest) {
                        draw_vp_overlay(
                            frame,
                            &price_to_y,
                            region.x,
...
fn draw_clusters(
    ...
    // Stacked-imbalance zones (drawn once per candle when the Imbalance study is on)
    if let Some((threshold, _, ignore_zeros)) = imbalance {
        draw_stacked_imbalance(
            frame,
            &price_to_y,
            footprint,
            step,
...
/// Minimum consecutive same-side imbalanced cells to flag a "stacked" zone.
const STACK_IMBALANCE_MIN: usize = 3;

/// Diagonal imbalance side for a single price cell (Some(true)=buy, Some(false)=sell).
/// Mirrors the per-cell test in `draw_imbalance_markers`.
fn cell_imbalance_side(
    footprint: &KlineTrades,
    price: Price,
    step: PriceStep,
    threshold: usize,
    ...
    let sell_qty = footprint
        .trades
        .get(&price)
        .map_or(0.0, |g| g.sell_qty.to_f64());
    let higher = price.add_steps(1, step);
    let buy_qty = footprint
        .trades
        .get(&higher)
        .map_or(0.0, |g| g.buy_qty.to_f64());
    if ignore_zeros && (sell_qty <= 0.0 || buy_qty <= 0.0) {
        return None;
    }
    let factor = (100 + threshold) as f64 / 100.0;
    if buy_qty >= sell_qty {
        if buy_qty > sell_qty * factor {
            return Some(true);
        }
    } else if sell_qty > buy_qty * factor {
        return Some(false);
    }
    None
}
...
fn flush_stack_run(
    ...
    if run.len() < STACK_IMBALANCE_MIN {
        return;
    }
    let Some(is_buy) = side else { return };
    let top = price_to_y(*run.last().unwrap()) - cell_height / 2.0;
    let bot = price_to_y(*run.first().unwrap()) + cell_height / 2.0;
    let color = if is_buy {
        palette.success.base.color
    } else {
        palette.danger.base.color
    };
    // bright bracket just outside the candle marking the stacked-imbalance zone
    frame.fill_rectangle(
        Point::new(cell_left - 4.0, top),
        Size::new(3.0, (bot - top).abs()),
        color.scale_alpha(0.9),
    );
}

/// Highlight zones of ≥ `STACK_IMBALANCE_MIN` consecutive same-side imbalanced
/// cells — the classic ATAS "stacked imbalance" support/resistance signal.
#[allow(clippy::too_many_arguments)]
fn draw_stacked_imbalance(
    frame: &mut canvas::Frame,
    price_to_y: &impl Fn(Price) -> f32,
    footprint: &KlineTrades,
    step: PriceStep,
    ...
    if prices.len() < STACK_IMBALANCE_MIN {
        return;
    }
    prices.sort();

    let mut run_side: Option<bool> = None;
    let mut run: Vec<Price> = Vec::new();

    for p in prices {
        let side = cell_imbalance_side(footprint, p, step, threshold, ignore_zeros);
        let adjacent = run.last().is_some_and(|last| last.add_steps(1, step) == p);
        if side.is_some() && side == run_side && adjacent {
            run.push(p);
        } else {
            flush_stack_run(frame, price_to_y, &run, run_side, cell_height, cell_left, palette);
            run.clear();
            run_side = side;
            if side.is_some() {
                run.push(p);
