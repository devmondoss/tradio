...
impl canvas::Program<Message> for HeatmapChart {
    ...
    fn draw(
        ...
        let heatmap = chart.cache.main.draw(renderer, bounds_size, |frame| {
            ...
            // Liquidity walls (bid=support/green, ask=resistance/red) with anti-spoof.
            // Solid+bright = persisted (real); faint = fresh (possible spoof).
            if self
                .studies
                .iter()
                .any(|s| matches!(s, HeatmapStudy::LiquidityWalls))
            {
                let symbol = chart.ticker_info.ticker.to_string();
                if let Some(w) = crate::confluence::current_walls(&symbol) {
                    for (price, qty, persisted, color) in [
                        (w.bid_price, w.bid_qty, w.bid_persisted, palette.success.base.color),
                        (w.ask_price, w.ask_qty, w.ask_persisted, palette.danger.base.color),
                    ] {
                        if qty <= 0.0 {
                            continue;
                        }
                        let y = chart.price_to_y(exchange::unit::price::Price::from_f64(price));
                        let (h, alpha) = if persisted { (2.0, 0.8) } else { (1.0, 0.35) };
                        frame.fill_rectangle(
                            Point::new(region.x, y - h / 2.0),
                            Size::new(region.width, h),
